...
from train.semantic_and_depth.unsupervised_depth_supervised_semantic_two_ds import SupervisedSemanticUnsupervisedDepthTrainer
...
if __name__ == "__main__":
    ...
    if cfg.device.multiple_gpus:
        ...
        mp.spawn(run_trainer, nprocs=torch.cuda.device_count(),
                 args=(cfg, torch.cuda.device_count(), SupervisedSemanticUnsupervisedDepthTrainer,))
    else:
        # standart single gpu 'cuda:0' training
        depth_trainer = SupervisedSemanticUnsupervisedDepthTrainer(0, cfg)
        depth_trainer.run()
