...
from train.depth.train_supervised_depth import SupervisedDepthTrainer
...
if __name__ == "__main__":
    ...
    if cfg.device.multiple_gpus:
        ...
        mp.spawn(run_trainer, nprocs=torch.cuda.device_count(),
                 args=(cfg, torch.cuda.device_count(), SupervisedDepthTrainer,))
    else:
        # standart single gpu 'cuda:0' training
        depth_trainer = SupervisedDepthTrainer(0, cfg)
        depth_trainer.run()
