...
from train.semantic.supervised_semantic_single_ds import SupervisedSemanticTrainer
...
if __name__ == "__main__":
    ...
    if cfg.device.multiple_gpus:
        ...
        mp.spawn(run_trainer, nprocs=torch.cuda.device_count(),
                 args=(cfg, torch.cuda.device_count(), SupervisedSemanticTrainer,))
    else:
        # standart single gpu 'cuda:0' training
        depth_trainer = SupervisedSemanticTrainer(0, cfg)
        depth_trainer.run()
