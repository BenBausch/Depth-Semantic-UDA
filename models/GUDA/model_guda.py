...
class GudaMonodepth(ModelBase):
    def __init__(self, device, cfg):
        super(GudaMonodepth, self).__init__()

        self.num_layers_encoder = cfg.model.depth_net.params.nof_layers # which resnet to use e.g. num_layers_encoder = 101 --> resnet101
        ...
        # Specify depth and semantic network (Monodepth 2)
        self.networks["resnet_encoder"] = ResnetEncoder(
            self.num_layers_encoder, self.weights_init_depth_net_encoder == "pretrained")

        self.networks["depth_decoder"] = DepthDecoder(
            self.networks["resnet_encoder"].num_ch_enc, range(self.num_scales))
        ...
        if not self.no_cuda and self.multiple_gpus:
            ...
            self.networks["resnet_encoder"].to(device)
            self.networks["depth_decoder"].to(device)
            self.networks["semantic_decoder"].to(device)
    ...
    def predict_semantic(self, input):
        return None
...
if __name__=="__main__":
    ...
    print(net.networks['depth_encoder'].forward((next(iter(gta_dataset)))))
